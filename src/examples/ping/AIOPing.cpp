...
static option longOpts[] = {{"interval", required_argument, nullptr, 'i'},
...
void readCb(AsyncOpStatus status, aioObject *rawSocket, HostAddress address, size_t transferred, void *arg)
{
  ...
  // raw IPv4 sockets deliver the IP header in front of the ICMP message, raw IPv6 sockets deliver the ICMPv6 message alone (RFC 3542)
  ...
  if (status == aosSuccess && transferred >= (ipHeaderSize + sizeof(icmp))) {
    ...
    if (receivedIcmp->icmp_type == echoReplyType) {
      ...
      if (F != client->times.end()) {
        ...
        fprintf(stdout, " * [%u] response from %s %0.4lgms\n", static_cast<unsigned>(receivedIcmp->icmp_id), gTarget, diff / 1000.0);
...
void pingTimerCb(aioUserEvent *event, void *arg)
{
  ...
  // the ICMPv6 checksum needs a pseudo-header with source/destination addresses, so the kernel always computes it for raw ICMPv6 sockets
  if (clientData->remoteAddress.family != AF_INET6)
    clientData->data.icmp_cksum = InternetChksum(reinterpret_cast<uint16_t*>(&clientData->data), sizeof(icmp));

  aioWriteMsg(clientData->rawSocket, &clientData->remoteAddress, &clientData->data, sizeof(icmp), afNone, 1000000, nullptr, nullptr);
...
void printTimerCb(aioUserEvent *event, void *arg)
{
  ...
  ICMPClientData *clientData = static_cast<ICMPClientData*>(arg);
  for (std::map<unsigned, timeMark>::iterator I = clientData->times.begin(), IE = clientData->times.end(); I != IE; ++I) {
...
int main(int argc, char**argv)
{
  ...
  while ((res = getopt_long(argc, argv, shortOpts, longOpts, &index)) != -1) {
    switch (res) {
      case 0:
        ...
        break;
      case 'c': gCount = static_cast<unsigned>(atoi(optarg)); break;
      case 'i': gInterval = atof(optarg); break;
      case '4': gFamily = AF_INET; break;
      case '6': gFamily = AF_INET6; break;
      case ':': fprintf(stderr, "Error: option %s missing argument\n", longOpts[index].name); break;
      case '?': fprintf(stderr, "Error: invalid option %s\n", argv[optind - 1]); break;
      default: break;
    }
  }
  ...
  HostAddress remoteAddress;
  {
    ...
    if (err != 0 || !result) {
      fprintf(stderr, " * cannot retrieve address of %s (%s)\n", gTarget, err != 0 ? gai_strerror(err) : "no addresses returned");
      return 1;
    }
  ...
  inet_ntop(remoteAddress.family,
            remoteAddress.family == AF_INET6 ? static_cast<const void*>(remoteAddress.ipv6) : static_cast<const void*>(&remoteAddress.ipv4),
            addressText,
            sizeof(addressText));

  // ICMPv6 cannot reach v4-mapped ::ffff:a.b.c.d addresses; macOS getaddrinfo returns them for AF_INET6 requests when the host has no global
  // IPv6
  if (remoteAddress.family == AF_INET6 &&
      remoteAddress.ipv6[0] == 0 &&
      remoteAddress.ipv6[1] == 0 &&
      remoteAddress.ipv6[2] == 0 &&
      remoteAddress.ipv6[3] == 0 &&
      remoteAddress.ipv6[4] == 0 &&
      remoteAddress.ipv6[5] == 0xffff) {
    fprintf(stderr, " * %s has no reachable IPv6 address (resolver returned v4-mapped %s)\n", gTarget, addressText);
    return 1;
  }

  // route check and source address discovery, the trick system ping uses: connect() on a UDP socket only does a routing lookup - no datagram is
  ...
  HostAddress localAddress;
  {
    ...
    // a link-local source needs a scope id that HostAddress cannot carry yet; fall back to the wildcard address and let the kernel pick the
    // source for every packet, the way this example always worked
    if (localAddress.family == AF_INET6 && (localAddress.ipv6[0] & htons(0xffc0)) == htons(0xfe80))
      memset(localAddress.ipv6, 0, sizeof(localAddress.ipv6));
  }

  fprintf(stdout, "PING %s (%s)\n", gTarget, addressText);

  socketTy S = socketCreate(remoteAddress.family,
                            SOCK_RAW,
                            remoteAddress.family == AF_INET6 ? static_cast<int>(IPPROTO_ICMPV6) : static_cast<int>(IPPROTO_ICMP),
                            1);
  socketReuseAddr(S);
  if (socketBind(S, &localAddress) != 0) {
    char sourceText[INET6_ADDRSTRLEN];
    inet_ntop(localAddress.family,
              localAddress.family == AF_INET6 ? static_cast<const void*>(localAddress.ipv6) : static_cast<const void*>(&localAddress.ipv4),
              sourceText,
              sizeof(sourceText));
