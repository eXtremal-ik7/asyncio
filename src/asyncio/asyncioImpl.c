...
asyncBase *createAsyncBase(AsyncMethod method, unsigned loopThreads)
{
  ...
  // Each supported OS has exactly one backend, so the method parameter cannot select anything different from the OS default; it stays in the
  // signature for a platform with alternative backends.
  __UNUSED(method);
...
{
  // Level, not edge: the sticky word stops the threads already looping, the ones asleep in the kernel wait and the ones that have not entered
  // asyncLoop yet (see the litmus at asyncBase.quitRequested). One doorbell is enough - each exiting thread re-rings while others remain. The
  // RMW-plus-write pair keeps this callable from a signal handler: no queue traffic, no allocation, no lock.
...
{
  // Quiescent-only by contract: every asyncLoop returned and no concurrent postQuitOperation/asyncLoop/reset. The store stays atomic, so a
  // racing quit is not UB - it lands on the old round (erased here) or on the next one, unspecified which. Everything else the loops touched is
  // already in its entry state after a clean exit (slots and counter zero, horizons parked at UINTPTR_MAX, leftover queue items and armed
  // timers are next round's legitimate work; a residual doorbell costs one spurious wakeup).
  ...
  for (unsigned i = 0; i < base->loopThreadSlotWords; i++)
    assert(__uintptr_atomic_load(&base->loopThreadSlots[i], amoSeqCst) == 0 && "resetQuitOperation while a loop slot is still owned");
#endif
...
{
  // An operation that lost its status race is detached by the list walker; the racing party that won still releases it through here later. With
  // cleared links and not being the head it is provably not a member - unlinking it anyway would rewrite head/tail through stale pointers and
...
static void opArmTimer(asyncOpRoot *op)
{
  if (op->timeout) {
    ...
    if (op->flags & afRealtime) {
      ...
      // Keep the grid path in ticks throughout. Match the former saturating-microsecond semantics: UINT64_MAX us rounded up is the largest
      // representable deadline, not UINT64_MAX grid ticks.
      uint64_t nowTick = getMonotonicTicks();
      uint64_t timeout = op->timeout;
      uint64_t deltaTick = timeout / TIMER_TICK_MICROSECONDS + (timeout % TIMER_TICK_MICROSECONDS != 0);
      const uint64_t deadlineLimit = UINT64_MAX / TIMER_TICK_MICROSECONDS + (UINT64_MAX % TIMER_TICK_MICROSECONDS != 0);
      op->deadlineTick = nowTick >= deadlineLimit || deltaTick > deadlineLimit - nowTick ? deadlineLimit : nowTick + deltaTick;
...
// Complete an operation whose start node has not armed a timer or issued I/O. opRelease() deliberately is not used: a realtime operation owns a
// timer cell from allocation onward, but stopTimer is only legal after that cell was actually armed. The operation-specific release hook still
// has to run before the completion is published (large captured buffers may otherwise leak).
...
// One-shot transport initialization does not live in the read/write queues. Its slot is assigned by the combiner while processing the init
// start node; ordinary operations submitted afterwards may queue, but cannot start until initialization leaves the slot. Leaving the slot kicks
// both queues; a failure also cancels everything queued behind it with the same status.
...
static void initializationTryComplete(asyncOpRoot *op, AsyncOpStatus status, uint32_t *needStart)
{
  ...
  // Terminal: the operation (connect / composite handshake) is done - a progress signal brought us here, so the result stands even if a
  // concurrent timeout won the status race. Adopt our status if we win, otherwise keep the winner's terminal status, and release now: nothing
  // else will complete it (its child operations are already consumed, so a late cancelMethod would find nothing).
  opSetStatus(op, opGetGeneration(op), status);
  initializationRelease(op, opGetStatus(op), needStart);
}

// Re-drive the parked initialization operation from backend READ/WRITE progress. Its one-shot-before-I/O contract makes either positional bit
...
void processInitializationOp(aioObjectRoot *object, uint32_t *needStart)
{
  ...
  switch (combinerSelectInitializationAction(op->running, status)) {
    ...
    case ciaRelease:
      // Either a proactor completion arrived after cancellation, or a child completion made a composite handshake terminal. In both cases the
      // I/O which owned the slot is done; do not re-run executeMethod.
...
static void cancelInitializationOp(aioObjectRoot *object, AsyncOpStatus status)
{
  ...
  if (opSetStatus(op, opGetGeneration(op), status)) {
    if (op->running == arRunning) {
      ...
      // The slot is assigned only while its start node is being processed, so a visible waiting operation has no timer or I/O in flight and is
...
// The one bulk-cancel of everything parked on an object: the initialization slot and both queues. Shared by the three sweep positions (CANCELIO
// tag, DELETE tag, DeletePending at ownership release), which must stay in sync.
...
// Start an op-node captured by the combiner (submission). The former processAction(aaStart): the initialization-slot owner is armed and driven
// in place, an ordinary operation is queued; the combiner reconciles the rest by status.
void startOperation(asyncOpRoot *op, uint32_t *needStart)
{
  aioObjectRoot *object = op->object;

  // Central rejection for a genuinely unstarted node. arRunning operations may already own child/kernel I/O and must enter their normal
  // queue/slot so the existing cancel path can retain them until the late completion.
  if (op->running == arWaiting && __uint_atomic_load(&object->DeletePending, amoRelaxed)) {
    opReleaseUnstarted(op, aosCanceled);
    return;
  }

  if (op->opCode & OPCODE_INIT) {
    // Only the combiner owner touches the slot. A duplicate init request is a client error, but its operation lifecycle still has to complete
    // safely.
    ...
    opArmTimer(op);
    if (opGetStatus(op) != aosPending) {
      // The arm itself expired the operation (its deadline window was already swept): the timeout won before the I/O started, so never issue
      // the connect/handshake. Releasing here mirrors the pre-start branch above - the cancel signal's sweep will find the slot already empty
      initializationRelease(op, opGetStatus(op), needStart);
      return;
    }
...
// Survivor accumulator shared by the positional sweeps (reapQueue and cancelOperationList): survivors keep their relative order in a freshly
// linked queue, everything else was released in place. The caller clears the op's links before appending, so a released op never leaves a stale
// prev behind.
typedef struct {
  asyncOpRoot *head;
  asyncOpRoot *tail;
...
// Reap terminal operations from one queue positionally: release the ones with no I/O in flight, hold an in-flight op (cancelMethod == 0) on its
// head until its late completion. Pending operations are left untouched. The queue is rebuilt from survivors; if the new head is a
// ready-to-start operation, ask for a kick.
...
// CANCEL/CANCELIO reconcile: a cancel source (timeout/opCancel/cancelIo) has already set the terminal status (winner-takes) and asked the
// combiner to scan. Reap the terminals it left behind, positionally, across the initialization slot and both queues.
void reapObject(aioObjectRoot *object, uint32_t tag, uint32_t *needStart)
{
  // Positional cancelIo sweep: the CANCELIO bit rides the chain entry that was the head at cancelIo() time, and the backend starts that entry's
  // operation before calling here - everything submitted before the call is already in the queues and gets swept, entries pushed after it sit
  // above the bit and survive
  ...
  if (ex) {
    ...
    switch (combinerSelectReapAction(ex->running, status)) {
      ...
      case craKeep:
        // Pending, or already cancelling an in-flight operation: wait for its normal/late READ or WRITE progress.
        break;
    }
  }
...
{
  asyncOpRoot *op = list->head;
  // Both queues are frozen while transport initialization is in flight; they are kicked when it leaves the slot.
  ...
  while (op) {
    ...
    AsyncOpStatus status = opGetStatus(op);
    if (status != aosPending) {
      // Already terminal - a progress completion set it (a PROGRESS_* signal brought us here, so the I/O is done) or it was cancelled while
      // queued. Finish without re-issuing: re-running executeMethod on a proactor would post a fresh overlapped I/O for a completed operation.
      opRelease(op, status, 0);
      op = next;
      continue;
    }

    // Dying object: nothing new may start. A run-to-completion submission would finish inside this very pass, dodge the ownership-release sweep
    // (it only reaps queued operations) and let a flood pin the object past objectDelete forever; leave it queued for the sweep to cancel
    ...
    // The syscall has completed, so this position is ours to release even if a concurrent timeout/cancel won the terminal status CAS. Its
    // signal is positional and cannot release the operation after we advance the queue; preserve the winner's status, but never drop the
    // completed operation.
    opSetStatus(op, opGetGeneration(op), status);
  ...
  list->head = op;
  if (op)
    // The dropped prefix was released without unlinking; a stale prev left here would send a later eqRemove writing into recycled memory
    op->executeQueue.prev = 0;
  else
    list->tail = 0;
}
...
{
  // Positional-aware bulk cancel (cancelIo / DeletePending / disconnect / connect-fail cascade). An in-flight operation whose cancelMethod
  // returns 0 (a proactor abort request the kernel owns until its completion arrives) must stay on its head so the late positional PROGRESS_*
  // signal still finds and releases it - dropping it would strand the operation and, on objectDelete, leak the object whose reference it holds.
  // By the one-in-flight invariant at most the head survives; the queue is rebuilt from the survivors rather than wiped.
  asyncOpRoot *op = list->head;
  SurvivorList survivors = {0, 0};
  while (op) {
    asyncOpRoot *next = op->executeQueue.next;
    ...
    if (opSetStatus(op, opGetGeneration(op), status)) {
      ...
      // Status race lost: the winner (a concurrent completion/timeout) drives the release through its own reconcile scan. Leave the operation
      // in the queue - the signal no longer carries the op pointer, so the queue is the only way that scan can still find it.
      keep = 1;
    }
...
int opCancel(asyncOpRoot *op, uintptr_t generation, AsyncOpStatus status, aioObjectRoot *object, uintptr_t objectGeneration)
{
  ...
  if (!object)
    return 1;
  // op storage may recycle immediately after the winning status CAS. The arm handle is the only legal route to its owner from this point
  // onward.
...
{
  // Progress from a child completion: always signal the parent, best-effort status on failure. Never gate the signal on the status CAS - a
  // child completion must still release a parent a concurrent timeout put into arCancelling (invariant 1/5).
  if (status != aosSuccess)
    opSetStatus(op, opGetGeneration(op), status);
...
TimerLoopState *loopThreadEnter(asyncBase *base)
{
  ...
  for (unsigned id = 0; id < base->loopThreadLimit; id++) {
    ...
    // Loop entry is cold, but there is no reason to issue an RMW against every occupied bit. The relaxed prefilter may only cause a harmless
...
unsigned loopThreadExit(asyncBase *base)
{
  ...
  // Publish awake before making the index reusable. The active counter drops while the bit is still owned, so a new entrant cannot be included
  // in the returned quit-propagation count by racing reuse of this exact slot.
  __uintptr_atomic_store(&base->timerLoopStates[id].wakeTick, UINTPTR_MAX, amoRelaxed);
...
{
  // The cancelIo sweep lives in reapObject(), driven by the position of the CANCELIO bit in the captured chain - an eager sweep here would run
  // before the captured submissions are started and let an operation submitted before the cancelIo() call escape it
  if (tag & COMBINER_TAG_DELETE) {
    cancelAllObjectOperations(object);
    // Ownership serializes the only writer. Generation is an identity token, not a publication channel, so a relaxed load/store avoids a locked
    // RMW.
...
// One captured Head entry: the shared DELETE handling runs first - it destroys the object and the drain must stop (non-zero return, nothing may
// touch the object afterwards) - otherwise the backend task handler runs the node/tag.
static inline int combinerDispatch(aioObjectRoot *object, combinerTaskHandlerTy *taskHandler, asyncOpRoot *op, uint32_t tag)
{
  if (combinerTaskHandlerCommon(object, tag))
    return 1;
  // Pair a child/kernel completion with the sticky delete sweep before the backend consumes its sole progress signal. A retained cancelling
  // head is then released by that same pass instead of waiting for a wake already used.
  if ((tag & COMBINER_TAG_PROGRESS_MASK) && __uint_atomic_load(&object->DeletePending, amoRelaxed))
    cancelAllObjectOperations(object);
...
void combiner(aioObjectRoot *object, AsyncOpTaggedPtr stackTop, AsyncOpTaggedPtr forRun)
{
  ...
  for (;;) {
    ...
    while ((currentHead.data = __uint64_atomic_load(&object->header.tag.low, amoRelaxed)) == stackTop.data) {
      // A dying object is swept once more at every ownership-release point: this is the only position that is guaranteed to come after every
      // action of the captured chains, so a submission that slipped past the positional CANCELIO sweep cannot survive the delete and pin the
      // object (sticky DeletePending; the sweep is idempotent, re-runs only cost a re-check)
    ...
    while (!__uint64_atomic_compare_and_swap(&object->header.tag.low, currentHead.data, stackTop.data, amoSeqCst))
      // Still only a CAS expected. The successful capture CAS above acquires the chain before the first next-pointer is read.
      currentHead.data = __uint64_atomic_load(&object->header.tag.low, amoRelaxed);

    // The captured chain is linked newest to oldest (each push points to the previous head); running it as is would invert single-thread
    // submission order. Reverse the operation nodes first. A no-op node (stub or bare counter tag) has no next field, so it can only be the
    // chain tail; it keeps running after the operations pushed on top of it.
    ...
    // Run dequeued tasks in submission order. The tail (a stub or a bare counter tag with no next field) is the oldest entry of the chain, so
    // its tags run before the operations pushed on top of it - a CANCEL bit OR-ed onto it must not sweep submissions that arrived after the
    // cancel. A DELETE tag destroys the object: nothing may touch it afterwards, including the head CAS of the next loop turn - return
    // immediately. Nothing gets abandoned by that: the tag is pushed on the refcount hitting zero, every not-yet-finished operation holds a
    // reference, so an operation action can never legally sit behind the DELETE tag
    if (tail.data) {
      asyncOpRoot *current;
      uint32_t tag;
