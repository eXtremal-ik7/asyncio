// Anchor TU for the post-build asm guard (cmake/atomic128-guard.cmake): forces the inline atomic128 layer into a standalone object so
// nm/objdump can prove the DWCAS stayed inline (no silent libatomic fallback) and that TSan builds route it through __tsan_atomic128_*.
#include "atomic128.h"
...
int atomic128GuardCasRelaxed(volatile uint128 *ptr, uint128 *expected, uint128 desired)
{
