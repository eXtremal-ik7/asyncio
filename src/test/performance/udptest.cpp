...
struct Context {
  ...
  uint16_t port;
  Context(uint16_t portArg) :
    totalPacketNum(gTotalPacketNum),
    groupSize(gGroupSize),
    messageSize(gMessageSize),
    port(portArg) {}
};
...
static const char *aioSenderName[] = {"blocking", "async", "coroutine"};

static const char *aioReceiverName[] = {"blocking", "async", "async+timer", "async+timer+rt", "coroutine"};
...
void *test_sync_sender(void *arg)
{
  ...
  for (uint64_t i = 0; i < senderCtx->config->totalPacketNum; i++) {
    if (sendto(senderCtx->clientSocket, msg, senderCtx->config->messageSize, 0, reinterpret_cast<sockaddr*>(&destAddr), sizeof(destAddr)) ==
        -1) {
...
void test_aio_writecb(AsyncOpStatus status, aioObject *object, size_t transferred, void *arg)
{
  ...
  while (senderCtx->counter < senderCtx->config->totalPacketNum) {
    ssize_t result =
        aioWriteMsg(object, &address, &senderCtx->buffer, senderCtx->config->messageSize, afActiveOnce, 0, test_aio_writecb, senderCtx);
    if (result > 0)
      senderCtx->counter++;
    else
      return;
...
void *test_sync_receiver(void *arg)
{
  ...
  while (!receiverStopping(receiverCtx)) {
    ...
    int result = recvfrom(receiverCtx->serverSocket, msg, sizeof(msg), 0, reinterpret_cast<sockaddr*>(&addr), &len);
...
static void receiverAccountPacket(ReceiverCtx *ctx)
{
  if (!ctx->started) {
    ...
    // Keep the interval valid even when a deliberately short diagnostic run receives fewer packets than one publication group.
...
void test_readcb(AsyncOpStatus status, aioObject *socket, HostAddress address, size_t transferred, void *arg)
{
  __UNUSED(transferred);
  __UNUSED(address);
  threadPacketsNum++;
  ...
  // Drain the socket buffer inline, mirroring the sender at test_aio_writecb: afActiveOnce reports every buffered datagram through the return
  // value, one completion per budget window comes back through this callback and restarts the drain
  while (!receiverStopping(ctx) && aioReadMsg(socket, &ctx->buffer, sizeof(ctx->buffer), afActiveOnce, 0, test_readcb, ctx) > 0) {
    threadPacketsNum++;
    receiverAccountPacket(ctx);
  }
}

// Asynchronous receiver with timer callback; serves both the plain and the realtime flavor - the RT context adds afRealtime to every re-arm.
// One predictable branch per packet next to a syscall does not affect the rates.
void test_readcb_timer(AsyncOpStatus status, aioObject *socket, HostAddress address, size_t transferred, void *arg)
{
  __UNUSED(address);
  __UNUSED(transferred);
  threadPacketsNum++;
  ...
  if (status == aosSuccess) {
    ...
    while (!receiverStopping(ctx) &&
           aioReadMsg(socket, &ctx->buffer, sizeof(ctx->buffer), static_cast<AsyncFlags>(rt | afActiveOnce), 1000000, test_readcb_timer, ctx) >
               0) {
      threadPacketsNum++;
      receiverAccountPacket(ctx);
    }
...
void *test_aio_receiver(void *arg)
{
  ...
  threadPacketsNum = 0;
  switch (ctx->type) {
    case aioReceiverAsync: aioReadMsg(ctx->server, &ctx->buffer, sizeof(ctx->buffer), afNone, 0, test_readcb, ctx); break;
    case aioReceiverAsyncTimer: aioReadMsg(ctx->server, &ctx->buffer, sizeof(ctx->buffer), afNone, 1000000, test_readcb_timer, ctx); break;
    case aioReceiverAsyncRT: aioReadMsg(ctx->server, &ctx->buffer, sizeof(ctx->buffer), afRealtime, 1000000, test_readcb_timer, ctx); break;
    default: fprintf(stderr, "Invalid receiver type, exiting...\n"); exit(1);
  }
...
static void wakeBlockingReceivers(uint16_t port, unsigned receiverThreads)
{
  ...
  char byte = 0;
  for (unsigned i = 0; i < receiverThreads; i++) {
    // A dropped wake datagram (EINTR/ENOBUFS under load) leaves one receiver blocked in recvfrom forever and the join below never returns.
    while (sendto(wakeSocket, &byte, sizeof(byte), 0, reinterpret_cast<sockaddr*>(&address), sizeof(address)) != sizeof(byte))
...
void test_aio(unsigned senderThreads, unsigned receiverThreads, uint16_t port, AIOSenderTy senderTy, AIOReceiverTy receiverTy)
{
  if (gDebug)
    fprintf(stderr,
            "BEGIN port=%u S/R=%u/%u sender=%s receiver=%s\n",
            port,
            senderThreads,
            receiverThreads,
  ...
  Context config(port);
  // Normal benchmarks sample time once per large group to keep clock reads out of the packet hot path. A small command-line smoke run may
  // distribute fewer than one group to each receiver; sample every packet in that case so progress and elapsed time remain meaningful instead
  // of leaving endPt before the first-packet beginPt.
  ...
  for (unsigned i = 0; i < receiverThreads; i++) {
    ...
    switch (receiverTy) {
      case aioReceiverBlocking: receiverPool.emplace_back(test_sync_receiver, &allReceivers[i]); break;
      case aioReceiverAsync:
      case aioReceiverAsyncTimer:
      case aioReceiverAsyncRT: receiverPool.emplace_back(test_aio_receiver, &allReceivers[i]); break;
      case aioReceiverCoroutine: receiverPool.emplace_back(test_coroutine_receiver, &allReceivers[i]); break;
    }
  }
  ...
  for (unsigned i = 0; i < senderThreads; i++) {
    ...
    switch (senderTy) {
      case aioSenderBlocking: senderPool.emplace_back(test_sync_sender, &allSenders[i]); break;
      case aioSenderAsync: senderPool.emplace_back(test_aio_sender, &allSenders[i]); break;
      case aioSenderCoroutine: senderPool.emplace_back(test_coroutine_sender, &allSenders[i]); break;
    }
  }
  ...
  if (receiverTy == aioReceiverBlocking) {
    ...
    // Cancellation callbacks observe stopping and do not re-arm reads. The destructor fires after the last operation callback and posts the
    // quit marker, so every loop remains available to finish its own coroutine and drain the object before it is joined.
  ...
  printf("Threads S/R %u/%u sender=%s, receiver=%s, total messages: %" PRIu64
         ", packet lost: %.2lf%%, elapsed time: %.3lf, rate: %.3lf msg/s\n",
         senderThreads,
         receiverThreads,
         aioSenderName[senderTy],
         aioReceiverName[receiverTy],
  ...
  // Let the previous run fully wind down (worker threads exiting, in-flight datagrams draining, deferred object reclamation) before the next
  // one starts, so TSan does not attribute its leftovers to the next run
  std::this_thread::sleep_for(std::chrono::seconds(1));
}
