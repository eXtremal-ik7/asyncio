// White-box tests of the timeout machinery: the grid contract (timer_grid), the lock-free cascading wheel protocol (timer_wheel), the sleep
// handshake of the message loops with the grid (timer_wakeup), the kernel-armed realtime timers (timer_realtime) and the reactor doorbell
// protocol of epoll/kqueue (timer_reactor). Mocks come from coretest.h.
...
// Detach a slot's list without delivering it: diagnostic recovery for tests that deliberately leave a link behind the checkpoint
...
// Heap link for direct wheel insertion: the sweep recycles links into the global link pool, so a stack instance would leave a dangling pointer
// for a later addToTimeoutQueue
...
// Mounts a stack cell as the operation's realtime timer the way the backends create one. Stack instances must mirror the alignment of
// production cells (alignedMalloc) at their declaration: alignas(TAGGED_POINTER_ALIGNMENT).
...
// Wires a stack event/timer pair the way newUserEvent and the first arm do: the pairing is permanent, tag.low holds one reference and tag.high
...
// Publication helpers, one per arm flavor. Each reproduces the exact store sequence and memory order of the corresponding backend arm; the
// ident flavors also write the composite kernel id into fd.
...
TEST(timer_grid, rounds_deadlines_up_and_delivers_each_window_once)
{
  ...
  // The sweep to tick 10 delivers only the tick-9 window; a repeated sweep is a checkpoint no-op and cannot deliver the same window twice
...
// Terminal-window regression: the sweep reopens window 100 and publishes checkpoint 101, then the producer resumes and arms a deadline-100
// timer. The publication must be refused (the producer observes the swept window) and the operation expired immediately instead of being
// delivered one rotation late.
TEST(timer_grid, late_arm_after_swept_checkpoint_expires_instead_of_being_stranded)
{
  ...
  // If the diagnostic drain recovered the physical link, it must not remain in the global link pool with a pointer to this stack operation.
  if (stranded) {
...
TEST(timer_grid, current_or_stale_clock_reading_is_a_noop)
{
  ...
  // A caller whose clock reading is behind the confirmed cursor (it raced a faster sweeper) must not sweep anything or move the cursor
  // backwards
...
TEST(timer_grid, ordinary_operation_arms_monotonic_grid_timer)
{
  ...
  backend.drainCompletions();
  // The armed link stays parked in the wheel with a now-stale generation; the TestBackend teardown recycles it without touching this stack
  // operation
  objectDelete(&object.root);
}
...
TEST(timer_grid, huge_timeout_saturates_instead_of_wrapping)
{
  ...
  // UINT64_MAX must mean "practically never": unsaturated now + timeout wraps to nowUs - 1 and fires at once
  combinerPushOperation(&op.root);
  EXPECT_EQ(opGetStatus(&op.root), aosPending);
...
TEST(timer_grid, timeout_saturates_at_documented_maximum)
{
  TestBackend backend;
  TestObject object(backend);
  // The public contract: usTimeout saturates at MAX_TIMEOUT_US right at op initialization, so every backend arm (wheel, timerfd, kevent,
  // waitable timer) stays inside its kernel range instead of wrapping there. The assert must precede the push: the grid arm repurposes the
  // union slot as the absolute deadline tick.
  TestOp op(object, OPCODE_READ, afNone, UINT64_MAX);
  op.setResults({aosPending});
  ...
  // The realtime arm hands op->timeout to the backend as-is: the clamped period is what reaches the kernel
...
TEST(timer_wheel, detach_reopens_and_is_idempotent_per_incarnation)
{
  ...
  // The winner takes the chain, and the same CAS reopens the slot for the next rotation
  ...
  // Any other visitor of the same window observes the advanced baseTick: no chain, no second reopen
  EXPECT_EQ(timerWheelDetach(&backend.base, nullptr, 0, 5), nullptr);
  EXPECT_EQ(peekWheelSlot(backend, 0, 5).high, 5u + TIMER_WHEEL_SLOTS);

  // Publication into the drained window is refused: its deadlines are terminal, and the wheel must not accept a link that could only fire a
  // rotation late
  link->next = nullptr;
  EXPECT_EQ(timerWheelInsert(&backend.base, link, 5), 0);
  EXPECT_EQ(peekWheelSlot(backend, 0, 5).low, 0u);

  // The refused link stays with its owner and is processed exactly as during a sweep: the timeout is delivered and the link recycled
...
TEST(timer_wheel, bitless_detach_uses_fast_path_without_preclear_bracket)
{
  ...
  // An empty slot still has to advance its incarnation, but there is no destructive occupancy clear to protect.
  EXPECT_EQ(timerWheelDetach(&backend.base, &backend.loopStates[3], 0, 4), nullptr);
  EXPECT_EQ(peekWheelSlot(backend, 0, 4).high, 4u + TIMER_WHEEL_SLOTS);
  EXPECT_EQ(backend.loopStates[3].preclearSequence, 0u);
  EXPECT_EQ(backend.base.timerPreclearOverflow, 0u);

  // A live bitless chain models a publisher stalled between its pair CAS and occupancy set. The same fast DWCAS must acquire the chain safely
  // without opening a bracket.
  TestObject object(backend);
  TestOp op(object);
  asyncOpListLink *link = makeHeapLink(op, object, opGetGeneration(&op.root), 5);
  ...
  asyncOpListLink *chain = timerWheelDetach(&backend.base, &backend.loopStates[3], 0, 5);
  ASSERT_EQ(chain, link);
  EXPECT_EQ(backend.loopStates[3].preclearSequence, 0u);
  EXPECT_EQ(backend.base.timerPreclearOverflow, 0u);
  timerWheelProcessDetached(&backend.base, chain, 5);
...
TEST(timer_wheel, arm_behind_multiple_rotations_expires_without_publication)
{
  ...
  TestObject object(backend);
  TestOp op(object);
  // The producer was preempted between reading the cursor and publishing for two whole rotations of the deadline's slot: the slot alone proves
  // the window is long gone, whatever the stale cursor hint says
  EXPECT_EQ(timerWheelDetach(&backend.base, nullptr, 0, 5), nullptr);
...
TEST(timer_wheel, publication_descends_into_open_lower_window_of_drained_upper)
{
  ...
  TestObject object(backend);
  TestOp op(object);
  // The sweep legally passed the level-1 boundary: the [1024, 2048) window is drained, but tick 1030 inside it is still ahead of the cursor
  processTimeoutQueue(&backend.base, nullptr, 1025);
  ASSERT_EQ(backend.base.timerCloseCursor, 1025u);

  // A producer with a stale cursor hint routes to the drained level-1 window first; the publication must descend into the still-open level-0
  // slot - not be refused and not park a rotation late in level 1
...
TEST(timer_wheel, expired_arm_never_starts_initialization_io)
{
  ...
  op.setResults({aosPending});
  // A connect whose timeout elapsed before the submission reached the combiner: the deadline computed from the monotonic clock lies behind the
...
TEST(timer_wheel, helper_confirms_cursor_and_stalled_owner_keeps_its_chain)
{
  ...
  // Sweeper A wins the tick-100 visit and stalls before both the confirm CAS and the processing of its detached chain
  ...
  // Sweeper B's tick-100 visits are idempotent no-ops, yet B confirms the cursor on A's behalf and the ticks behind the stalled one stay alive
...
TEST(timer_wheel, resurfaced_sweeper_with_stale_tick_cannot_rewind_cursor)
{
  ...
  // A sweeper that stalled before its confirm CAS resurfaces long after its tick was confirmed by helpers: the visits are idempotent no-ops and
  // the tick-exact confirm CAS misses, a plain checkpoint store would rewind here
...
TEST(timer_wheel, boundary_deadline_migrates_and_fires_in_its_exact_tick)
{
  ...
  // Against cursor 0 both deadlines differ in the bits 10..19 group, so both are routed to level 1 (the [1024, 2048) window slot)
  ...
  // Tick 1024 visits level 1 before level 0: the boundary deadline expires in this very tick (not one rotation later), the in-window one
  // migrates down into its level-0 slot
  processTimeoutQueue(&backend.base, nullptr, 1025);
...
TEST(timer_wheel, numeric_distance_keeps_adjacent_boundary_deadline_in_level_zero)
{
  ...
  // XOR sees a differing bit in the next radix group here. Numeric distance is one tick, so no level-1 publication/cascade is needed.
  op.root.deadlineTick = 1024;
  addToTimeoutQueue(&backend.base, &op.root);
  auto *link = static_cast<asyncOpListLink*>(op.root.timerId);
  ASSERT_EQ(peekWheelSlot(backend, 0, 0).low, reinterpret_cast<uint64_t>(link));
  EXPECT_EQ(peekWheelSlot(backend, 1, 1).low, 0u);
...
TEST(timer_wheel, cascade_chain_migrates_l2_to_l1_to_l0_and_fires_in_its_exact_tick)
{
  ...
  // Both deadlines differ from cursor 0 in the bits 20..29 group: level-2 routing. The boundary one sits exactly on the window start; the
  // chained one needs the full migration ladder level 2 -> level 1 -> level 0
  ...
  // Catching up a whole level-1 rotation in one call (a base resuming from suspend) delivers nothing ahead of time
  processTimeoutQueue(&backend.base, nullptr, l2Window);
  EXPECT_EQ(opGetStatus(&boundary.root), aosPending);
  EXPECT_EQ(opGetStatus(&chained.root), aosPending);

  // Tick 2^20 visits level 2: the boundary deadline expires in this very tick (not one rotation later), the chained one migrates into the
  // level-1 window reopened by the sweep in exact lockstep
  ...
  // The level-1 boundary tick migrates it again, into the level-0 slot whose current incarnation covers exactly the deadline tick
...
TEST(timer_wheel, boundary_deadline_at_top_level_window_start_fires_at_its_visit)
{
  ...
  TestObject object(backend);
  TestOp op(object);
  // Differs from cursor 0 in the bits 30..39 group: top-level routing, into the [2^30, 2^31) window whose slot still holds its initial
  // incarnation
  ...
  // Sweeping 2^30 ticks one by one is out of unit-test reach; the visit units run directly at the exact window-start tick, as the contiguous
  // sweep would. The window-start deadline is due at this very visit and must be delivered here, not re-parked for another rotation
...
TEST(timer_wheel, beyond_range_deadline_clamps_to_last_representable_point)
{
  ...
  TestObject object(backend);
  TestOp op(object);
  // The deadline is exactly one full range ahead. Routing clamps the target point to cursor + range - 1, the last point represented by the
  // wheel.
  ...
  // Still pending and parked: the wheel teardown recycles the link without delivering it
  objectDecrementReference(&object.root, 1);
  objectDelete(&object.root);
}
...
TEST(timer_wheel, near_deadline_across_range_boundary_is_delivered_not_clamped)
{
  ...
  // One tick ahead, but across the 2^40 binary boundary - only the XOR image looks out-of-range
...
TEST(timer_wheel, far_insert_with_stale_cursor_refuses_already_swept_deadline)
{
  ...
  // Stale hint (allowed by contract) from before the boundary, deadline already behind the confirmed sweep: the insert must refuse, not park
...
TEST(timer_wheel, stalled_cascade_into_swept_window_delivers_exactly_once)
{
  ...
  // The owner wins the level-1 boundary visit and stalls before processing its chain; helpers sweep past the deadline meanwhile, so the level-0
  // slot the cascade is aiming at is already reopened for its next rotation
  ...
  // The resumed cascade observes the terminal window and must deliver right now (late, never early - the deadline tick is already swept), not
  // park the link into the reopened incarnation to fire a rotation later
...
TEST(timer_wheel, terminal_same_generation_link_is_dropped_before_recascade)
{
  ...
  ASSERT_EQ(peekWheelSlot(backend, 1, 4).low, reinterpret_cast<uint64_t>(link));

  // Completion changed only the status bits. A generation-only check would re-publish this dead link into level 0 at the boundary.
  __uintptr_atomic_store(&op.root.tag, (generation << TAG_STATUS_SIZE) | aosSuccess, amoRelaxed);
  timerWheelProcessDetached(&backend.base, timerWheelDetach(&backend.base, nullptr, 1, 4096), 4096);
  EXPECT_EQ(opGetStatus(&op.root), aosSuccess);
  EXPECT_EQ(peekWheelSlot(backend, 0, 5000 % TIMER_WHEEL_SLOTS).low, 0u);
  EXPECT_FALSE(wheelSlotOccupied(backend, 0, 5000 % TIMER_WHEEL_SLOTS));

  objectDecrementReference(&object.root, 1);
  objectDelete(&object.root);
}

// The visitor pre-clears the occupancy bit before its drain CAS, so losing the drain race can leave the clear ordered after a fresh publication
// into the reopened incarnation. The idempotent exit must repair that: a live chain observed behind an advanced baseTick gets its bit re-set
// before the visitor backs off empty-handed.
TEST(timer_wheel, stale_detach_restores_live_occupancy_bit)
{
  ...
  // Simulate the lost race: the stale visitor's pre-clear landing after the fresh publication
  __uintptr_atomic_fetch_and(&backend.base.timerWheel.occupancy[0][0], ~(static_cast<uintptr_t>(1) << 5), amoSeqCst);

  // The stale visitor re-reads the pair, sees the advanced incarnation with a live chain and restores the bit while claiming nothing
...
TEST(timer_wakeup, occupancy_bit_follows_link_lifecycle)
{
  ...
  // Activating an empty slot sets its bit; stacking onto a live chain re-asserts it (every publisher performs the idempotent set itself).
  ...
  // A refused late arm never becomes visible: no bit and no kick, the timeout is delivered by the arm itself
...
TEST(timer_wakeup, stacked_arm_is_visible_to_sleep_despite_stalled_first_publisher)
{
  ...
  // Producer P1 activates the empty level-0 slot 5 and stalls between the publish CAS and the occupancy set - the exact first half of
  ...
  // Producer P2 stacks onto the live chain through the production path and returns: from here on the armed timer must not depend on P1 resuming
  stackedOp.root.deadlineTick = 5;
  addToTimeoutQueue(&backend.base, &stackedOp.root);
  EXPECT_TRUE(wheelSlotOccupied(backend, 0, 5));

  // Sleep planning reads only the bitmap: a bitless chain would park the loop in an eternal wait with both timers lost until P1 resumes
...
TEST(timer_wakeup, stalled_visitor_between_clear_and_drain_does_not_hide_due_chain)
{
  ...
  // A timer parked at tick 5 through the production path; the sweep catches up to its tick without visiting it yet
  op.root.deadlineTick = 5;
  addToTimeoutQueue(&backend.base, &op.root);
  ...
  // The tick-5 visitor opens its bracket, performs the occupancy clear and stalls before the drain CAS: the slot still holds the live due
  // chain, the bitmap no longer shows it
  __uintptr_atomic_fetch_and_add(&backend.base.timerPreclearOverflow, timerPreclearOverflowEntry, amoSeqCst);
  __uintptr_atomic_fetch_and(&backend.base.timerWheel.occupancy[0][0], ~(static_cast<uintptr_t>(1) << 5), amoSeqCst);
  ASSERT_NE(peekWheelSlot(backend, 0, 5).low, 0u);

  // Another loop planning its sleep sees the open bracket and must not trust its empty scan: an eternal wait here would park the loop until the
  // stalled visitor resumes
  EXPECT_EQ(timerLoopPrepareSleep(&backend.base, &backend.loopStates[0], 5), 6u);
  EXPECT_EQ(backend.loopStates[0].wakeTick, 6u);
  timerLoopCancelSleep(&backend.loopStates[0]);

  // The woken loop drains the tick as a helper; the stalled visitor is not needed for delivery
  processTimeoutQueue(&backend.base, nullptr, 6);
  EXPECT_EQ(opGetStatus(&op.root), aosTimeout);
  EXPECT_EQ(peekWheelSlot(backend, 0, 5).low, 0u);
  __uintptr_atomic_fetch_and_add(&backend.base.timerPreclearOverflow, ~static_cast<uintptr_t>(0), amoRelease);
...
TEST(timer_wakeup, stalled_boundary_visitor_does_not_hide_cascading_window)
{
  ...
  // Tick 5000 parks in level-1 slot 4 (window [4096, 5120)); the sweep stops right at the window's visit boundary
  ...
  // The boundary visitor opens its bracket, clears the level-1 bit and stalls before its drain CAS; the cascade the sleeper counts on has not
  // happened
  __uintptr_atomic_fetch_and_add(&backend.base.timerPreclearOverflow, timerPreclearOverflowEntry, amoSeqCst);
  __uintptr_atomic_fetch_and(&backend.base.timerWheel.occupancy[1][0], ~(static_cast<uintptr_t>(1) << 4), amoSeqCst);

  // The boundary window still holds the chain behind the open bracket: the sleeper must wake right behind the cursor instead of waiting
  // eternally
  ...
  // The woken helper performs the visit and the stalled visitor resumes (losing CAS, closed bracket): the chain cascades into level 0 with its
...
TEST(timer_wakeup, stale_clear_of_reopened_slot_caps_sleep_while_visitor_is_parked)
{
  ...
  // The tick-1023 visitor loads the pair {*, baseTick 1023} and stalls before its occupancy pre-clear (executed below); a helper sweeps the
  ...
  // The stale visitor resumes inside its still-open bracket and its pre-clear lands on the NEW incarnation's bit (the bit carries no
  // incarnation number); it stalls again before its losing CAS
  __uintptr_atomic_fetch_and_add(&backend.base.timerPreclearOverflow, timerPreclearOverflowEntry, amoSeqCst);
  __uintptr_atomic_fetch_and(&backend.base.timerWheel.occupancy[0][1023 >> 6], ~(static_cast<uintptr_t>(1) << (1023 & 63)), amoSeqCst);

  // CONTRACT: the open bracket forbids committing to an eternal wait - the sleep is capped at one grid tick however long the visitor stays
  // parked, so delivery does not hinge on the guilty thread's schedule
  ...
  // The visitor's resume: the losing CAS refreshes the pair, the repair re-sets the bit, the bracket closes
...
TEST(timer_wakeup, sleep_snapshot_excludes_only_the_callers_own_state)
{
  ...
  // The owning loop cannot be inside detach and sleep planning at once, so its own sequence is intentionally absent from both snapshots.
  ...
  // Every other loop remains visible. An open bracket makes the one-tick result final without spending a full bitmap scan.
  backend.loopStates[1].preclearSequence = 1;
  EXPECT_EQ(timerLoopPrepareSleep(&backend.base, &backend.loopStates[0], 0), 1u);
...
TEST(timer_wheel, detach_bracket_routes_by_visitor_identity)
{
  ...
  // A foreign thread (no loop identity) brackets through the overflow word: one completed entry stays recorded in the high half
  ...
  timerWheelProcessDetached(&backend.base, timerWheelDetach(&backend.base, &backend.loopStates[3], 0, 6), 6);
  EXPECT_EQ(opGetStatus(&second.root), aosTimeout);
...
TEST(timer_wakeup, cascade_reinsert_kicks_sleepers_that_would_miss_the_deadline)
{
  ...
  // A sleeper due at tick 5001 meets the tick-5000 deadline: the cascade at boundary 4096 re-parks the link without a kick
  ...
  // A sleeper due at tick 20000 would deliver the tick-9200 deadline over ten thousand ticks late: the re-inserting sweeper is this link's
  // publisher and must kick, exactly like a fresh arm - a sleeper that scanned before the level-0 bit appeared has no other way to learn of it
...
TEST(timer_wakeup, producer_kicks_only_sleepers_that_would_miss_the_deadline)
{
  ...
  // A sleeper due to wake at tick 10 would deliver a tick-5 deadline five ticks late: the arm must kick it
  backend.loopStates[2].wakeTick = 10;
  first.root.deadlineTick = 5;
  addToTimeoutQueue(&backend.base, &first.root);
  EXPECT_EQ(backend.wakeupCalls, 1u);

  // A sleeper waking at tick 6 sweeps every tick below it and delivers the tick-5 deadline itself with at most a tick of lag: no kick
  backend.loopStates[2].wakeTick = 6;
  second.root.deadlineTick = 5;
  addToTimeoutQueue(&backend.base, &second.root);
  EXPECT_EQ(backend.wakeupCalls, 1u);

  // Awake threads park their slots and never attract kicks: they re-scan the occupancy bitmap before their next sleep
...
TEST(timer_wakeup, prepare_sleep_is_tickless_and_wakes_right_after_nearest_deadline)
{
  ...
  // Empty wheel: the wait is unbounded and the slot carries the eternal sentinel - farther than any tick, so any arm kicks
  EXPECT_EQ(timerLoopPrepareSleep(&backend.base, &backend.loopStates[0], 0), UINT64_MAX);
  EXPECT_EQ(backend.loopStates[0].wakeTick, timerSleepEternal);
  timerLoopCancelSleep(&backend.loopStates[0]);
  EXPECT_EQ(backend.loopStates[0].wakeTick, UINTPTR_MAX);

  // A faraway deadline sleeps up to the visit of its slot, without polling: tick 5000 parks in level-1 slot 4, whose window [4096, 5120) is
  // visited at tick 4096 (the cascade boundary must not be overslept)
  ...
  // A deadline inside the level-0 rotation shrinks the wait to its exact wake tick (deadline + 1: the sweep of a tick runs on the first call
  // past it)
  ...
  // Delivery clears the near slot and the sleep stretches back out to the far link's cascade boundary
  processTimeoutQueue(&backend.base, nullptr, 6);
...
TEST(timer_wakeup, prepare_sleep_scans_the_full_rotation_in_wrap_around_order)
{
  ...
  // A deadline beyond the wheel range clamps into the farthest future top-level slot: index 1023, one position BEHIND the scan's start - only
  // the circular wrap-around pass sees its bit (a horizon-capped scan never could). Its visit is millennia out, so the wait meets the
  // kernel-range clamp; the published horizon matches the actual wake, waking early just re-scans
...
TEST(timer_wakeup, prepare_sleep_clamps_far_wakes_to_the_kernel_timeout_range)
{
  ...
  // Tick 2^25 parks on level 2 (slot 32); its visit is ~48 days away, past what epoll accepts as an int millisecond timeout. The wait is
  // clamped and the published horizon matches the actual wake
...
TEST(timer_wakeup, prepare_sleep_wakes_at_cascade_boundary_of_occupied_upper_slot)
{
  ...
  // Distance 1027 selects level 1. Deadline 2049 lives in slot 2 until the tick-2048 visit re-cascades it: a sleeper spanning that boundary
  // must wake right after the boundary tick or the cascade (and the deadline) oversleeps
  ...
  // Past the boundary the link sits re-cascaded in its exact level-0 slot and the next sleep runs up to the deadline itself
...
// Regression for the opEncodeTag removal (kqueue/ident protocol): a stale doorbell - harvested into another loop thread's batch, then processed
// after the operation completed, its storage got recycled and the SAME timer got re-armed - must not adopt the new arming's generation and win
// the status CAS. The arming identity travels full-width through the kernel as the knote ident; a mismatch with the published tag rejects the
// doorbell.
TEST(timer_reactor, stale_doorbell_must_not_expire_a_rearmed_operation)
{
  ...
  // The first incarnation arms its realtime timer; the kernel keeps both the envelope and the ident of this arming inside the harvested event
  ...
  // The operation completes; its storage is recycled and re-armed for a new submission with a fresh deadline before the doorbell gets processed
...
// Same scenario through the epoll/deadline protocol. A stale envelope cannot consume shared fd state: it snapshots the current absolute
// deadline. Before that deadline it is ignored; afterwards expiring the current generation is correct regardless of which envelope caused the
// wakeup.
...
// User events restart with an UNCHANGED operation generation, so the arming identity - not the generation - must distinguish restarts (the old
// "compare timer and event tag" TODO). kqueue/ident protocol: every arming takes a fresh ident, a doorbell of the previous one no longer
// matches.
...
// Same restart through the epoll/timerfd protocol: a loop delivery snapshots only the current schedule. The owner later reads timerfd under its
// serial ownership; eventtest covers the EAGAIN/confirmed-expiration decisions.
...
// The "tag == 0 means unpublished" sentinel must not collide with a legally wrapped generation. On a 32-bit target the generation field is 24
// bits wide: after 2^24 recycles of one operation slot initAsyncOpRoot wraps it to zero - the armed encoding must stay disjoint from the
// sentinel anyway, or every delivery reads "stale doorbell" and the timeout is dropped
...
TEST(timer_reactor, arming_a_wrapped_generation_must_not_publish_the_unpublished_sentinel)
{
  ...
  // The slot's generation legally wraps around to zero (2^24 recycles on a 32-bit target)
...
#ifdef __linux__
// The kernel dependency the probe/owner protocol stands on: timerfd_settime with a new value RESETS the fd's expiration counter, so the owner
// reading after a restart gets EAGAIN instead of inheriting the previous schedule's expiration. If this ever changes, a stale probe could spend
// a new counter.
TEST(timer_reactor, timerfd_settime_resets_the_expiration_counter)
{
  ...
  // Re-arm with a distant deadline WITHOUT reading the expired count first - exactly what a recycle+rearm does while a stale doorbell sits in a
  // batch
...
// Every timer publication uses the same compact objectHeader envelope as objects and user events. The immutable header type, rather than
// pointer tag bits, selects the timer decoder after the address/generation expansion.
TEST(timer_reactor, every_timer_envelope_is_a_common_header_handle)
{
  ...
  // Kind is bound only at the first arm; timer construction itself does not read the operation's not-yet-initialized opCode.
  timer.header.timer.kind = tkOperation;
  ...
  // User-event timer kind lives in the common-header union and the schedule generation follows the same encoding.
  aioUserEvent event{};
