...
typedef ParserResultTy UriRangeParser(const char**ptr, const char *end, bool uriOnly, uriParseCb callback, void *arg);

// Expected URI::ipv6 layout: groups in the order they are written in the literal, each group in host byte order.
...
void expectTruncatedEscapeNeedsMore(UriRangeParser *parser, const char *source, size_t size)
{
  // Keep the allocation exact: there is deliberately no readable NUL byte after end, so ASan catches either byte of percent-escape lookahead.
...
int captureQuery(URIComponent *component, void *arg)
{
  ...
  if (component->type == uriCtQueryElement) {
    capture->elements.push_back(
        std::make_pair(std::string(component->raw.data, component->raw.size), std::string(component->raw2.data, component->raw2.size)));
...
TEST(uriparse, full_parsers_honor_callback_cancellation)
{
  ...
  const CancelCase uriCases[] = {{"http://user@example.com:8080/path?x=1#f", uriCtSchema},
  ...
  const CancelCase hostPortCases[] = {{"example.com:8080", uriCtHostDNS},
...
TEST(uriparse, valid_pct_escape_at_exact_range_end)
{
  struct TestCase {
    ...
    int componentType;
  } cases[] = {{uriParsePath, uriCtPath}, {uriParseQuery, uriCtQuery}, {uriParseFragment, uriCtFragment}};
  ...
  for (const TestCase &test: cases) {
    ...
    ASSERT_EQ(test.parser(&ptr, source + sizeof(source), true, captureRawComponent, &capture), ParserResultOk);
...
TEST(uriparse, path_accepts_every_pchar_class)
{
  ...
  ASSERT_EQ(uriParsePath(&ptr, source + sizeof(source) - 1, false, captureRawComponent, &path), ParserResultOk);
...
TEST(uriparse, query_elements_continue_after_ampersand)
{
  ...
  ASSERT_EQ(uriParseQuery(&ptr, source + sizeof(source) - 1, false, captureQuery, &capture), ParserResultOk);
  EXPECT_EQ(ptr, source + sizeof(source) - 2);
...
{
  URI uri;
  ASSERT_EQ(uriParse("http://%41%42x%00@example.com/%43%44x%00?q=%45%46#%47%48", &uri), 1);

  const char expectedUserInfo[] = {'A', 'B', 'x', '\0'};
  const char expectedPath[] = {'/', 'C', 'D', 'x', '\0'};
  EXPECT_EQ(uri.userInfo, std::string(expectedUserInfo, sizeof(expectedUserInfo)));
...
// Malformed input may be rejected or leniently accepted, but the parser must not crash or hang on it.
...
// F22: a host starting with a digit is a reg-name unless the whole host is a valid dotted quad (RFC 3986)
...
// The authority port of the full uriParse must follow the same rules as the uriParseHostPort pair: values beyond 65535 are rejected (never
// accumulated into signed overflow), and an empty port keeps the "-1 = no explicit port" sentinel so build() cannot turn "host:/" into
// "host:0/". An explicit ":0" stays a real port, matching the hostport parser.
...
// parse source, build, compare with the canonical form, then re-parse the built string and require component-level equality with the first
// parse
void expectRoundTrip(const char *source, const char *canonical)
{
  ...
  URI reparsed;
  ASSERT_EQ(uriParse(built.c_str(), &reparsed), 1) << source << " built as " << built;
  expectUriFieldsEqual(uri, reparsed);
}

} // namespace

// RFC 5952 canonical form: lowercase hex, no leading zeros, the longest run of two or more zero groups compressed to "::", leftmost run on a
// tie
TEST(uriparse, build_roundtrip_ipv6_hosts)
{
  ...
  expectRoundTrip("http://[2001:0db8:0001:0002:0003:0004:0005:0006]", "http://[2001:db8:1:2:3:4:5:6]");
...
TEST(uriparse, build_omits_absent_port)
{
  ...
  expectRoundTrip("http://example.com", "http://example.com");
  expectRoundTrip("http://user@example.com/a/b?x=1#f", "http://user@example.com/a/b?x=1#f");
}
...
// high bytes must be encoded from their unsigned value: 0xD0 is "%D0", not the sign-extended garbage "%40"
TEST(uriparse, build_pct_encodes_high_bytes)
{
  expectRoundTrip("http://example.com/%D0%BF?q=%D0%96#%D1%8F", "http://example.com/%D0%BF?q=%D0%96#%D1%8F");
  expectRoundTrip("http://us%D0%AFer@example.com/", "http://us%D0%AFer@example.com/");
}
